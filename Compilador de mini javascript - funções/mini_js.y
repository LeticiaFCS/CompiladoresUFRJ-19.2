...
%token NUM NEGNUM STR ID LET NEWOBJ NEWARRAY IF ELSE IGUAL MAI_IG MEN_IG RETURN FUNCTION ASM
...
START : {empilha_escopo();}CMD
      ;

CMD : LET Decl CMD
	| P CMD
	| IF_LINHA CMD
	| E ASM ';' { for(auto i:$2.v)New_Inst(i);
	New_Line();} CMD 
	|FUNCTION_CALL ';' {New_Line();}CMD
	| FUNCTION ID{ 
	   ...
	   {desempilha_escopo();New_Line();}CMD
	  | E ';' CMD 
	  |
	;

FUNCTION_CALL : ID '('PARAM_CHA')' {New_Inst($1.v); New_Inst(get); New_Inst(callFunc);}
	          ;
...
LVALUEPROP : E '.' LVALUEPROPSUFFIX
		   ...
		   | E'['E']'
		   ;
LVALUEPROPSUFFIX: FIM_LVALUE
				| FIM_LVALUE {New_Inst(getProp);}'['ELPROP']'
				| FIM_LVALUE'.'{New_Inst(getProp);}LVALUEPROPSUFFIX
				| FIM_LVALUE'.'{New_Inst(getProp);}LVALUEPROPSUFFIX'['ELPROP']'
				;
...
PARAM_FUN : ARGs_FUN 
	  | 
	  ;
ARGs_FUN : ARGs_FUN ',' ID {$$.id+=1; lista_arg($3.v[$3.v.size()-1],$$.id);}
	 | ID {
	 	$$.id=0;
	 	lista_arg($$.v[$$.v.size()-1],$$.id);
		}
	 ;
...
CMD_FUN : LET Decl_FUN CMD_FUN
	...
	| E_FUN ASM ';' { for(auto i:$2.v)New_FInst(i);
	New_Func_Line();} CMD_FUN 
	| RETURN RVALUE_FUN {
...
PARAM_CHA_FUN : ARGs_CHA_FUN {New_FInst(to_string($$.id));}
	  | 
	  ;
